...
export const Config = Schema.object({
  ...
  // Keep the full-size upper bound while allowing compact bubbles down to 40%.
  bubbleScale: Schema.number().min(0.4).max(1.2).step(0.05).default(1).role('slider').description('气泡大小'),
  ...
  voiceEnabled: Schema.boolean().default(true).description('启用语音播报（macOS 系统语音）'),
...
function mount(ctx, config = {}, eventCtx = ctx) {
  ...
  const resolveApiKey = async () => {
    ...
    // Do not parse the plaintext credentials file here. The launcher owns
    // the credential binding and the visual pet does not need an API key;
    // optional roast/balance actions work only when DSH explicitly exposes a
    // credential through its credentials service.
    return apiKey
  }
