...
// 把宠物/气泡的屏幕坐标矩形上报给主进程（macOS 点击穿透切换用）。
// getBoundingClientRect 是窗口内坐标，需要加上窗口在屏幕上的偏移。
function reportPetRects() {
  if (!window.petBridge || typeof window.petBridge.reportRect !== 'function') return
  const rects = []
  const toScreen = (r) => ({
    x: r.left + (window.screenX || 0),
    y: r.top + (window.screenY || 0),
    width: r.width,
    height: r.height,
  })
  const hit = hitEl.getBoundingClientRect()
  if (hit.width > 0 && hit.height > 0) rects.push(toScreen(hit))
  if (bubbleEl.classList.contains('visible')) {
    const b = bubbleEl.getBoundingClientRect()
    if (b.width > 0 && b.height > 0) rects.push(toScreen(b))
  }
  if (menuEl.classList.contains('visible')) {
    const m = menuEl.getBoundingClientRect()
    if (m.width > 0 && m.height > 0) rects.push(toScreen(m))
  }
  window.petBridge.reportRect(rects)
}

// macOS 上窗口进入忽略状态后收不到 mousemove，无法实时更新 lastMouse；
// 用定时器兜底上报矩形，保证主进程的光标轮询拿到的区域始终是最新的。
setInterval(reportPetRects, 250)
