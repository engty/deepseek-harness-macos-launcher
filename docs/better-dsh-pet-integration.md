...
桌宠只作为状态展示和动画窗口使用。上游 Windows 版的语音识别依赖 SAPI，当前
macOS 适配版不启用麦克风识别、桌宠内置任务执行和直接读取明文凭据文件；这些
Windows 专用入口不会伪装成可用功能。语音播报已改用 macOS 自带的 `/usr/bin/say`
（默认中文音色 `Tingting`，找不到时回退系统默认音色），只在明确的互动回复和
任务结果气泡中播报，不上传音频，也不新增第三方语音引擎。主 App 的 DeepSeek
API Key 与余额查询仍由启动器自己的 Keychain/DSH 凭据流程负责。
